...
#로지스틱 회귀 이진 분류(2가지 경우(양성or음성)에 대해서만 분류), 이진분류는 로지스틱 함수(=시그모이드 함수) 이용해 확률 출력
...
print(lr.predict(train_bream_smelt[:5])) #처음 5개에 대한 로지스틱 회귀 이진분류 실행
...
#C값 조정으로 노름 규제 조절(C 커지면 규제 약해짐(복잡해짐), C 작아지면 규제 강해짐), max_iter(반복횟수)를 늘려서 반복횟수 오류 방지
